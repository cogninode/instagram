...
//             body: JSON.stringify({ task_type: taskType, target: target })
//         });
        
//         const data = await response.json();
        
//         if (data.success) {
//             showNotification('Task created successfully! Scraping started.', 'success');
...
//             accountsList.innerHTML = '<p>No accounts added yet. Add your first Instagram account above.</p>';
//             return;
//         }
...
//                     <td>${acc.last_used ? new Date(acc.last_used).toLocaleString() : 'Never'}</td>
//                 </tr>
//             `;
//         });
...
//                     <td>
//                         ${task.status === 'completed' ? `<button class="btn btn-small" onclick="viewTaskData(${task.id})">View Data</button>` : '-'}
//                     </td>
//                 </tr>
//             `;
//         });
...
//         dataWindow.document.write('<html><head><title>Task Data</title><style>body{font-family:Arial;padding:20px;background:#f5f5f5}pre{background:white;padding:20px;border-radius:8px;overflow:auto}</style></head><body>');
//         dataWindow.document.write('<h1>Scraped Data</h1>');
...
//             document.getElementById('task-form').reset();
//             document.getElementById('max-items').value = 50; // Reset to default
//             loadStats();
...
//                             ? `<button class="action-btn" onclick="viewTaskData(${task.id})">📊 View Data</button>` 
//                             : task.status === 'running'
...
//                         a.href = url;
//                         a.download = 'task-${taskId}-data.json';
//                         a.click();
//                     }
